...
public:
    ...
    Color(Vector3 fromVector3)
    {
        r = (unsigned char)fromVector3.x;
        g = (unsigned char)fromVector3.y;
        b = (unsigned char)fromVector3.z;
    }
    Color(COLORREF fromColorRef)
    {
        b = (fromColorRef >> 16) & 0xFF;
        g = (fromColorRef >> 8) & 0xFF;
        r = fromColorRef & 0xFF;
    }
    ...
    {
        return Color((long)(value * r), (long)(value * g), (long)(value * b));
    }
    Color operator / (const float& value)
    {
        return Color((long)(value / r), (long)(value / g), (long)(value / b));
    ...
    {
        return Color((long)r + (long)second.r, (long)g + (long)second.g, (long)b + (long)second.b);
    }
    Color operator - (const Color& second)
    {
        return Color((long)r - (long)second.r, (long)g - (long)second.g, (long)b - (long)second.b);
    ...
    {
        return Color(Vector3::Lerp(a.toVector3(), b.toVector3(), t));
    }
...
public:
    ...
    Material(Shader shaderType, float Metallic, float Roughness)
    {
        shader = shaderType;
        metallic = Metallic * 0.75f;
        roughness = Roughness;
        metal = Color(108, 107, 117);
    }
};
...
typedef struct Vertex3
{
    ...
    Vertex3(const Vector3& pos)
    {
        position = pos;
        color = Color();
    }
    Vertex3(const Vector3& pos, const Color& col)
    {
        position = pos;
        color = col;
    }
...
typedef struct Triangle
{
    ...
    {
        a = Vertex3(A, color), b = Vertex3(B, color), c = Vertex3(C, color);
    }
    ...
    {
        return Vector3(a.position.x + b.position.x + c.position.x,
...
public:
    ...
    Mesh(const std::vector<Vector3>& vertexList, const std::vector<unsigned>& triangleList)
    {
        vertices = vertexList;
    ...
    void AddScale(const Vector3& scale)
    {
        ...
        for (size_t i = 0; i < vertSz; ++i)
        {
            vertices[i].MultiplyPairwise(scale);
        }
    }
    ...
    static Mesh GenerateCuboid(const Vector3& size)
    {
        ...
        nCube.triangles = { 0, 2, 1, 1, 2, 3, 2, 6, 3, 3, 6, 7, 3, 7, 1, 1, 7, 5, 4, 5, 6, 6, 5, 7, 5, 4, 1, 1, 4, 0, 6, 2, 4, 4, 2, 0 };
    ...
    static Mesh GenerateIcoSphere(const float& radius)
    {
        ...
        nIco.triangles = { 0, 1, 4,  0, 4,  9, 9, 4, 5,  4,  8, 5, 4, 1, 8, 8, 1,  10, 8, 10, 3, 5, 8, 3,  5, 3, 2, 2, 3,  7,
                          7, 3, 10, 7, 10, 6, 7, 6, 11, 11, 6, 0, 0, 6, 1, 6, 10, 1,  9, 11, 0, 9, 2, 11, 9, 5, 2, 7, 11, 2 };
        nIco.vertices = { Vector3(-sX, 0, sZ), Vector3(sX, 0, sZ),  Vector3(-sX, 0, -sZ), Vector3(sX, 0, -sZ),
